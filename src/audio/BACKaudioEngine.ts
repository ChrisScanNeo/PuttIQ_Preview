...
export class AudioEngine {
  ...
  async stop() {
    ...
    if (this.windPlayer) {
      try {
        this.windPlayer.pause();
        this.windPlayer.remove();
      } catch (error) {
  ...
  private async startWindBackground() {
    ...
    try {
      ...
      // Create looping wind player - pass object with uri property
      this.windPlayer = createAudioPlayer({ uri: windSprite.uri });
      this.windPlayer.volume = this.gain * 0.3; // Wind at 30% volume
      this.windPlayer.loop = true;
      this.windPlayer.play();
    } catch (error) {
  ...
  private async playClip(sprite: any, clipInfo: any, gainDb: number) {
    try {
      // Create a new player for this clip - pass object with uri property
      const player = createAudioPlayer({ uri: sprite.uri });
      const playerId = `${Date.now()}_${Math.random()}`;
      this.activePlayers.set(playerId, player);

      // Set volume using the property, not a method
      const linearGain = this.dbToLin(gainDb) * this.gain;
      player.volume = linearGain;

      // Seek to clip start and play
      player.seekTo(clipInfo.start);
      player.play();
